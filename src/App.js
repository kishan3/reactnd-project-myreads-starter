...
class BooksApp extends React.Component {
  ...
  handleStausChange = (book, status) => {
    console.log(book, status, 'lol')
    const books = this.state.books.filter((e) => e.name !== book.name)
    book.status = status
    this.setState(() => ({
        books : [...books, book]
      }))
    ...
    return (
      <div className="app">
        ...
        <Route exact path="/" render={() => (
          <ListBooks
            books={this.state.books}
            onStatusChange={(book, status) => {this.handleStausChange(book, status)}}
          />
        )} />
      </div>
    )
